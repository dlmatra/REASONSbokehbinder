...
df = pd.read_pickle('./static/REASONS_DataFrame_withsdbinfo')
...
def create_figure():
    ...
    df['imgs']=[glob.glob('./static/'+x+'*.png') for x in df['Target']]
    #source.add([glob.glob('./static/'+x+'*.png') for x in source.data['Target']],
               #name='imgs')
